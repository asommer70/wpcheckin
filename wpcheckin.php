...
function wpcheckin_get_places($location) {
  ...
  $params = "&radius=10&types=gym|health|school|plumber|park|movie_theater|store|grocery_or_supermarket|department_store|casino|art_gallery|atm|airport|food|restaurant|university|liquor_store|night_club|bar|subway_station&sensor=true";

  $places_url = $base_url . $location . $params . $key;
  ...
  if ($dev_mode and isset($options['wpcheckin_places_feed'])) {
    ...
    $places_feed->status = 'OK';
    $places_status = $places_feed->status;
  }

  // If error from Google setup an empty $places_feed object.
  if ($places_status == 'REQUEST_DENIED' or $places_status == 'OVER_QUERY_LIMIT') {
  ...
  if ($places_status != 'REQUEST_DENIED' and $places_status != 'OVER_QUERY_LIMIT') {
    if (!$dev_mode) {
...
function wpcheckin_post_checkin() {
  ...
  $content .= "<br/>". esc_html($_POST['address']) ."<br/>". esc_html($_POST['phone']) ."<br/><a target='_blank' href='". esc_html($_POST['website']) ."'>". esc_html($_POST['website']) ."</a><br/>";
